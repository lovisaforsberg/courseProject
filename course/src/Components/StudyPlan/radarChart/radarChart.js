...
function flat (data, allCourses){
    data.children.map(item =>{
        if('children' in item){
			flat(item, allCourses)
		}
		else{
...
function createAxesAll(dataset){
	const data_array = []
	flat(dataset, data_array)
    let axes = []
...
function createAxesZoom(all, zoomed){
	const data_array = []
	flat(zoomed, data_array)
	let axes_zoom = []
	let axes = []
    ...
    data_array.map(course =>{
        course.allInfo.subjects.map(sub =>{
			if(sub !== "Technology"){
			add(axes_zoom, sub, course.size)
			//axes_zoom[sub] = course.size
			}
		})
	})
	let obj = {}
	axes_zoom.map(sub=>{obj[sub.axis]=sub.value})

	all.map(sub=>{
		if(sub.axis in obj){
			add(axes, sub.axis, obj[sub.axis])
		}
		else{
			add(axes, sub.axis, 0)
		}
...
function createData(all, Zoomed){
    ...
    let all_data = [
        ...
        {name: 'zoomed in courses',
        axes: createAxesZoom(all, Zoomed)
    }
    ]
return all_data
...
const RadarChart = function RadarChart(parent_selector, data, options) {
	...
	let g = svg.append("g")
			...
			.attr("transform", "translate(" + (cfg.w/2 + cfg.margin.left) + ",280)");
...
var margin = { top: 50, right: 80, bottom: 50, left: 80 },
                ...
                var radarChartOptions2 = {
                    ...
                    levels: 6,
                    roundStrokes: false,
                    color: d3.scaleOrdinal().range(["#E59A65", "#BB81C4"]),
                      format: '.0f',
                      legend: { title: 'Subjects', translateX: 100, translateY: 40 },
                      unit: 'hp'
                  };
      
    // Draw the chart, get a reference the created svg element :
    let svg_radar2 = RadarChart(d3Container.current, subjectData, radarChartOptions2);
